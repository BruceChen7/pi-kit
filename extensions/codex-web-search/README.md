...
This copy is vendored into `pi-kit` under `extensions/codex-web-search` and is registered by
default from the root `package.json`.
...
Inside `pi-kit`, no separate install step is required. The root `package.json` registers this
extension by default:

```json
{
  "pi": {
    "extensions": ["./extensions/codex-web-search"]
  }
}
```

For local development, reload Pi after changing the extension:

```text
/reload
```

Upstream package installs are still available from the original repository, but this vendored copy
is intended to be used through `pi-kit`.
