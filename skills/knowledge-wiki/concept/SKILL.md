...
- `../state/wiki-state.mjs` — state tracking (unprocessed summaries, last-run timestamps)
- `../state/wiki-index.mjs` — index management
...
```bash
node ../state/wiki-state.mjs find-unprocessed-summaries knowledge-wiki-concept --base-path "{KNOWLEDGE_PATH}"
```
...
1. Run:
   ```bash
   node ./wiki-concept.mjs create "{concept-slug}" "{Display Name}" --base-path "{KNOWLEDGE_PATH}"
   ```
...
2. Read the file using the Read tool, then edit it to add:
   ...
   **Language:** Always write concept articles in English, regardless of the language of the source summaries. Concepts are the cross-language synthesis layer of the knowledge base. Source summaries in this knowledge base are predominantly in Chinese — when a `description` field in the Key Concepts entry is in Chinese, keep it in Chinese; the article body you write should be in English. Use American English spelling (e.g. "organize" not "organise", "recognize" not "recognise").

3. Add the source link:
   ```bash
   node ./wiki-concept.mjs insert-source "{concept-slug}" "{summary-link-path}" --base-path "{KNOWLEDGE_PATH}"
   ```
...
```bash
node ../state/wiki-index.mjs upsert-concept "{concept-slug}" "{Display Name}" "{one-line English description}" --base-path "{KNOWLEDGE_PATH}"
```
...
```bash
node ../state/wiki-state.mjs set-last-run knowledge-wiki-concept --base-path "{KNOWLEDGE_PATH}"
```
