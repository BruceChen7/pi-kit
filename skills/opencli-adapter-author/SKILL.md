...
- Run OpenCLI commands with Pi's `bash` tool, for example `opencli list -f json`.
- Read files with `read`; search repository text with `rg`/`fd` rather than shell `grep`/`find`.
- When editing adapters or skill files, use Pi's `edit` for surgical changes and `write` only for new files or full rewrites.
- Respect the current Pi workflow: create TODOs in direct act mode, and follow plan/spec review gates when the change is logic- or process-changing.
