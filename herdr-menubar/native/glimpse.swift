...
// herdr-menubar extension: #RRGGBB → NSColor
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, WKScriptMessageHandler, NSWindowDelegate {
    ...
    func handleCommand(type: String, json: [String: Any]) {
        ...
        case "icon":
            // herdr-menubar extension: status item 彩色图标
            // { "type":"icon", "symbol":"circle.fill", "color":"#4ADE80" } 或
            // { "type":"icon", "image":"<base64 png>" }
            guard let button = nsStatusItem?.button else { break }
            let color = (json["color"] as? String).flatMap(nsColor(fromHex:))
            if let symbol = json["symbol"] as? String,
               let img = NSImage(systemSymbolName: symbol, accessibilityDescription: nil) {
                if let color {
                    button.image = img.withSymbolConfiguration(NSImage.SymbolConfiguration(paletteColors: [color]))
                } else {
                    button.image = img
                }
                button.image?.isTemplate = false
            } else if let b64 = json["image"] as? String, let data = Data(base64Encoded: b64) {
                button.image = NSImage(data: data)
                button.image?.isTemplate = false
            }
        case "resize":
