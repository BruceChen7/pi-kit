...
The tables are the truth. Every breadboard produces these. When writing a standalone
breadboard document, use the Pi-native path:

`.pi/plans/<repo>/shaping/YYYY-MM-DD-<topic>-breadboard.md`

When slicing a breadboard for implementation, write the slice output to:

`.pi/plans/<repo>/shaping/YYYY-MM-DD-<topic>-slices.md`

A breadboard may also be embedded in the active shaping document when it is only one section
of the current shape exploration.
