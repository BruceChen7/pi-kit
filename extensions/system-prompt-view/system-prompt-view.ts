...
import { Container, Key, Markdown, Text, matchesKey, type Component } from "@mariozechner/pi-tui";
...
class SystemPromptView implements Component {
		...
		this.container.addChild(
			new Text(
				theme.fg("accent", theme.bold("System Prompt")) +
					theme.fg("dim", ` (${this.lines.length} lines - Esc/q/Enter to close, j/k/arrows to scroll)`),
				1,
				0,
			),
		);
	...
	private updateContent() {
		...
		const visibleLines = this.lines.slice(this.scrollY, this.scrollY + visibleHeight);
		const content = visibleLines.join("\n");

		// Replace the Markdown component with a new one containing updated content
		// Markdown constructor: (content, x, y, theme, options?)
		this.container.children[this.bodyIndex] = new Markdown(content, 1, 0, this.markdownTheme);
...
function createMarkdownTheme(theme: any): any {
	...
	const dim = (text: string) => (typeof theme?.fg === "function" ? theme.fg("dim", text) : text);
	const accent = (text: string) => (typeof theme?.fg === "function" ? theme.fg("accent", text) : text);
