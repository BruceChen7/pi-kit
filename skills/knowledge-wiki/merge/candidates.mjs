 ...
 *
 * Usage:
 *   node skills/knowledge-wiki/merge/candidates.mjs find-shared-source-concepts
 *   node skills/knowledge-wiki/merge/candidates.mjs find-implied-parent-concepts
 *   node skills/knowledge-wiki/merge/candidates.mjs find-thin-concepts
 *
...
export function findImpliedParentConcepts() {
  ...
  for (const slug of slugs) {
    ...
    for (let len = parts.length - 1; len >= 1; len--) {
      ...
      if (slugSet.has(prefix)) {
        ...
        existingParentGroups[prefix].push({
  ...
  const clusters = [...newParentClusters, ...existingParentClusters].sort(
    (a, b) => {
      const depthDiff =
        b.impliedParent.split("-").length - a.impliedParent.split("-").length;
      return depthDiff !== 0
        ? depthDiff
...
export function findThinConcepts() {
  ...
  for (const entry of fs.readdirSync(CONCEPTS_DIR, { withFileTypes: true })) {
    ...
    if (wordCount < WORD_THRESHOLD && sources <= SOURCE_THRESHOLD) {
      concepts.push({
        _path: `Wiki/Concepts/${entry.name}`,
        _score: wordCount,
      });
    }
  }
...
function main() {
  ...
  if (!subcommand || subcommand === "--help") {
    console.error(
      "Usage: node skills/knowledge-wiki/merge/candidates.mjs <subcommand>",
    );
...
const selfArgv = process.argv[1];
if (selfArgv && import.meta.url === pathToFileURL(selfArgv).href) {
  main();
}
