...
description: >
  Scan the wiki for cross-cutting connections, implicit relationships, contradictions, and
...
- `./wiki-concept.mjs` — concept management (create, insert-connected-concept, insert-source)
- `./wiki-state.mjs` — state tracking (unprocessed summaries, last-run timestamps)
- `../state/wiki-index.mjs` — index management (read-concepts, upsert-concept)
...
```bash
node ./wiki-state.mjs find-unprocessed-summaries knowledge-wiki-synthesis --base-path "{KNOWLEDGE_PATH}"
```
...
1. Run:
   ```bash
   node ./wiki-concept.mjs create "{proposed_slug}" "{proposed_title}" --type Synthesis --icon notepad --base-path "{KNOWLEDGE_PATH}"
   ```
...
2. Read the file using the Read tool, then edit it to add:
   - **Article body** — insert between `# {proposed_title}` and `## Sources`: 3–5 paragraphs of synthesis in American English. Explain the connection, relationship, contradiction, or gap clearly. Note that source summaries in this knowledge base are predominantly in Chinese — when the synthesis draws on Chinese-language summaries, integrate the concepts using English terminology for the cross-language synthesis layer. Use American English spelling (e.g. "organize" not "organise", "recognize" not "recognise").
...
3. For each concept this synthesis draws from, add it to the Connected Concepts section (idempotent):
   ```bash
   node ./wiki-concept.mjs insert-connected-concept "{proposed_slug}" "{linked-slug}" "{Display Name}" --base-path "{KNOWLEDGE_PATH}"
   ```

4. For each source consulted, add its link (idempotent):
   ```bash
   node ./wiki-concept.mjs insert-source "{proposed_slug}" "{summary-path-without-md}" --base-path "{KNOWLEDGE_PATH}"
   ```
...
```bash
node ./wiki-concept.mjs insert-connected-concept "{linked-slug}" "{proposed_slug}" "{proposed_title}" --base-path "{KNOWLEDGE_PATH}"
```
...
```bash
node ../state/wiki-index.mjs upsert-concept "{proposed_slug}" "{proposed_title}" "{one-line English description}" --base-path "{KNOWLEDGE_PATH}"
```
...
```bash
node ./wiki-state.mjs set-last-run knowledge-wiki-synthesis --base-path "{KNOWLEDGE_PATH}"
```
