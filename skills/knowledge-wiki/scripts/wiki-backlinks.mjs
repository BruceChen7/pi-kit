...
export function updateAfterMerge(
  secondaryPath,
  primaryPath,
  primaryDisplayName,
) {
  ...
  const backlinkFiles = [
    ...new Set(
      links
        .filter((link) => link.to === secondaryPath)
        .map((link) => link.from),
    ),
  ].sort();
  ...
  for (const fromRel of backlinkFiles) {
    ...
    const connectedConceptsRange = getSectionRange(
      original,
      "Connected Concepts",
    );
    ...
    const result = lines.map((line, i) => {
      ...
      if (isPrimaryFile) {
        const inConnectedConcepts =
          connectedConceptsRange !== null &&
        ...
        return line.replace(
          secondaryLinkRe,
          (_, displayName) => displayName ?? secondarySlug,
        );
      }
      ...
      const primaryInSameList = block.some((blockLine) =>
        lineHasConceptLink(blockLine, primaryPrefix),
      );
