...
function resolveDefaultMaxSources(
  mode: SearchMode,
...
function resolveWebSearchInput(
  ...
  return {
    query,
    maxSources: normalizeMaxSources(
      input.maxSources,
      resolveDefaultMaxSources(mode, settings),
    ),
    mode,
...
function shouldUseDefuddleFallback(
  ...
  if (
    settings.defuddleMode !== "fallback" &&
    settings.defuddleMode !== "both"
  ) {
    return false;
  }
...
function hasDefuddleExtractionIntent(query: string): boolean {
  return /\b(defuddle|extract|clean|strip|markdown|summari[sz]e)\b/iu.test(
    query,
  );
}
...
function buildPromptStrategyHints(
  query: string,
  mode: SearchMode,
  queryBudget?: number,
): string[] {
  ...
  if (queryBudget !== undefined) {
    hints.push(
      `You have a hard limit of ${queryBudget} web search queries in this run.`,
    );
    hints.push(
  ...
  if (mode === "deep") {
    hints.push(
      "Stop once you have enough authoritative evidence to answer well.",
    );
  }
...
export function parseCodexWebSearchOutput(
  ...
  if (!isCodexWebSearchOutput(parsed)) {
    throw new Error(
      "Codex returned JSON that does not match the expected web search schema.",
    );
  }
...
export async function executeCodexWebSearch(
  ...
  const allowFastAutoEscalation = canAutoEscalateDefaultFastSearch(
    input,
    resolvedInput,
  );
  ...
  if (directUrl) {
    const directResult = await maybeRunDefuddleSearch(
      resolvedInput,
      options,
      settings,
      {
...
function canAutoEscalateDefaultFastSearch(
  ...
  return (
    originalInput.mode === undefined && originalInput.freshness === undefined
  );
}
...
async function runResolvedCodexWebSearch(
  ...
  emitProgressUpdate(
    options,
    progress,
    `Running ${mode} Codex web search for: ${query}`,
  );
  ...
  try {
    const runnerOptions: RunCodexCommandOptions = {
      ...
      stdin: buildCodexPrompt(
        { query, maxSources, mode },
        { queryBudget: policy.queryBudget },
      ),
      ...
      onStdoutLine: (line) => {
        ...
        for (const addedQuery of updates.queries) {
          emitProgressUpdate(
            options,
            progress,
            `Search #${progress.searchCount}: ${addedQuery}`,
          );
        }
    ...
    try {
      ...
      if (
        mode === "fast" &&
        !allowFastAutoEscalation &&
        shouldExhaustFastModeInTurn(failure)
      ) {
        markFastModeExhausted(options.turnState);
      }
      throw asError(failure, progress);
...
async function buildSoftFailureResult(
  ...
  const renderedResult = await renderToolResult(
    buildSoftFailureBody(summary, failure),
  );
...
> {
  if (
    !shouldUseDefuddleFallback(settings, input.query, defuddle.directUrlQuery)
  ) {
    return undefined;
  }

  const defuddleRunner = options.defuddleRunner ?? runDefuddleCommand;
  const urls = (defuddle.urls ?? getDefuddleFallbackUrls(input.query)).slice(
    0,
    input.maxSources,
  );
  if (urls.length === 0) {
  ...
  for (const url of urls) {
    emitProgressUpdate(
      options,
      progress,
      `Extracting clean page content with Defuddle: ${url}`,
    );
  ...
  const renderedResult = await renderToolResult(
    formatWebSearchResult({ summary, sources }),
  );
  const details: CodexWebSearchDetails = {
    query: input.query,
    mode: input.mode,
...
async function readFinalCodexOutput(
  outputPath: string,
  stdout: string,
): Promise<string> {
  let rawOutput = "";
  ...
  if (rawOutput.trim()) {
    return (
      extractJsonObjectCandidate(rawOutput) ??
      summary.finalAgentMessage ??
      rawOutput
    );
  }
...
function extractFinalAgentMessageFromEvent(
  ...
  for (let index = items.length - 1; index >= 0; index -= 1) {
    ...
    const message = extractAssistantMessageTextFromItem(
...
function extractErrorMessageFromEvent(
  event: Record<string, unknown>,
): string | undefined {
...
function extractAssistantMessageTextFromItem(
...
function shouldRetryWithDeepLiveSearch(
  ...
  if (
    originalInput.mode !== undefined ||
    originalInput.freshness !== undefined
  ) {
    return false;
  }
...
function classifyFailureText(message: string): CodexFailureDetails {
  if (
    /could not find `codex` in path|common install locations/i.test(message)
  ) {
    return createCodexFailure("missing_cli", message, false);
  }

  if (
    /failed to start codex cli|invalid .*config|config error/i.test(message)
  ) {
  ...
  if (
    /invalid JSON|does not match the expected web search schema/i.test(message)
  ) {
...
function markFastModeExhausted(
  turnState: WebSearchTurnState | undefined,
): void {
  if (turnState) {
...
function collectPageActions(
  ...
  for (const pageAction of pageActions) {
    ...
    const lastRecordedPageAction =
      recordedPageActions[recordedPageActions.length - 1];
    ...
    if (recordedPageActions.length > MAX_RECORDED_PAGE_ACTIONS) {
      recordedPageActions.splice(
        0,
        recordedPageActions.length - MAX_RECORDED_PAGE_ACTIONS,
      );
    }
...
async function renderToolResult(text: string): Promise<{
  ...
  const fullOutputDir = await mkdtemp(
    join(tmpdir(), "pi-codex-web-search-result-"),
  );
...
function shouldTreatTopLevelQueriesAsSearch(value: {
  ...
  const actionTypes = [
    extractActionType(value.action),
    extractActionType(value.output),
  ].filter(
...
function extractSearchActionQueries(value: unknown): string[] {
  ...
  if (typedValue.type === undefined || typedValue.type === "search") {
    return [
      ...extractQueryValues(typedValue.queries),
      ...extractQueryValues(typedValue.query),
    ];
  }
...
function isCodexWebSearchOutput(
  value: unknown,
): value is LooseCodexWebSearchOutput {
