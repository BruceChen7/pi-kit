...
- Keyboard shortcut `Ctrl+Alt+L` annotates the most recently modified generated plan file (`YYYY-MM-DD-*.md`) across the configured/default plan directories via shared event API (`action: "annotate"`), and now waits synchronously for the annotation result instead of using the short default request timeout.
