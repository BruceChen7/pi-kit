...
export default function codexWebSearchExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    renderCall(args, theme) {
      ...
      text += theme.fg(
        "dim",
        ` [${args.mode ?? "default"}/${args.freshness ?? "auto"}]`,
      );
      if (args.maxSources) {
    ...
    renderResult(result, { expanded, isPartial }, theme) {
      ...
      const pageActions = Array.isArray(details.pageActions)
        ? details.pageActions
        : [];
      ...
      if (!expanded) {
        ...
        const latestDetail = formatCollapsedResultDetail(details, pageActions);
        if (latestDetail) {
          text += `\n${theme.fg("dim", latestDetail)}`;
        }
  ...
  pi.registerCommand(SETTINGS_COMMAND, {
    ...
    getArgumentCompletions: (prefix) => {
      ...
      const matches = SETTINGS_ARGUMENT_OPTIONS.filter((option) =>
        option.startsWith(lowerPrefix),
      );
  ...
  try {
    switch (command) {
      ...
      case "fast-max-sources": {
        const fastMaxSources = parseInteger(
          value,
          1,
          MAX_ALLOWED_SOURCES,
          "fast max sources",
      ...
      case "deep-max-sources": {
        const deepMaxSources = parseInteger(
          value,
          1,
          MAX_ALLOWED_SOURCES,
          "deep max sources",
      ...
      case "default-max-sources": {
        ...
        notify(
          ctx,
          `Fast and deep max sources updated to ${saved.fastMaxSources}.`,
        );
        return;
      }
      ...
      case "defuddle-timeout-ms": {
        ...
        notify(
          ctx,
          `Defuddle timeout updated to ${saved.defuddleTimeoutMs} ms.`,
        );
        return;
      }
...
async function openSearchDefaultsDialog(
  ctx: ExtensionCommandContext,
): Promise<void> {
  while (true) {
    const settings = await loadSettings();
    ...
    if (choice.startsWith("Default mode")) {
      const mode = await ctx.ui.select(
        ...
        ["fast — quick lookup", "deep — broader research"],
      );
      if (!mode) continue;
      await handleSettingsCommand(
        `default-mode ${mode.startsWith("deep") ? "deep" : "fast"}`,
        ctx,
      );
      continue;
    }
...
async function openDefuddleSettingsDialog(
  ctx: ExtensionCommandContext,
): Promise<void> {
  while (true) {
    const settings = await loadSettings();
...
async function openTimeoutSettingsDialog(
  ctx: ExtensionCommandContext,
): Promise<void> {
  while (true) {
    const settings = await loadSettings();
...
async function openQueryBudgetSettingsDialog(
  ctx: ExtensionCommandContext,
): Promise<void> {
  while (true) {
    const settings = await loadSettings();
...
function parseDefuddleMode(value: string): DefuddleMode {
  ...
  throw new Error(
    `Invalid Defuddle mode: ${value}. Expected off, direct, fallback, or both.`,
  );
}
...
function formatCollapsedResultDetail(
  details: CodexWebSearchDetails,
  pageActions: string[],
): string | undefined {
  if (details.failure) {
    return formatInlineQuery(details.failure.message, 110);
  }
  if (details.latestQuery) {
    return `Last query: ${formatInlineQuery(details.latestQuery, 110)}`;
  }
  const lastPageAction = pageActions[pageActions.length - 1];
  return lastPageAction ? formatInlineQuery(lastPageAction, 110) : undefined;
}
...
function renderProgress(
  ...
  if (
    !details ||
    (details.searchQueries.length === 0 &&
...
function formatRetrySummary(retry: RetryProvenance): string {
  ...
  if (
    /reconnect|reconnecting|websocket|transport|stream disconnected/i.test(
