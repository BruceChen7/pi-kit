...
export async function runDefuddleCommand(
  ...
  return new Promise<DefuddleParseResult>((resolve, reject) => {
    const child = spawn(
      process.execPath,
      [cliPath, "parse", options.url, "--markdown", "--json"],
      {
        cwd: options.cwd,
    ...
    timeoutId = setTimeout(() => {
      ...
      finish(() => {
        reject(
          new Error(
            `Defuddle extraction timed out after ${Math.ceil(timeoutMs / 1000)} seconds.`,
          ),
        );
      });
    }, timeoutMs);
    ...
    child.on("error", (error) => {
      finish(() =>
        reject(new Error(`Failed to start Defuddle: ${error.message}`)),
      );
    });
    ...
    child.on("close", (code) => {
      if (code !== 0) {
        const message =
          stderr.trim() ||
          stdout.trim() ||
          `Defuddle exited with code ${code ?? 1}.`;
...
function parseDefuddleOutput(stdout: string, url: string): DefuddleParseResult {
  ...
  const content =
    pickString(output.contentMarkdown) || pickString(output.content);
  if (!content) {
