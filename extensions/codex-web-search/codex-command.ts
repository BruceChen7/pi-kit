...
export async function runCodexCommand(
  ...
  const triedMessage =
    tried.length > 0
      ? ` Checked: ${tried.map((path) => `\`${path}\``).join(", ")}.`
      : "";
...
export async function findBundledCodexExecutable(
  ...
  if (
    cachedBundledCodexPath &&
    (await isExecutableFile(cachedBundledCodexPath))
  ) {
...
function getCodexPackageRoots(cwd?: string): string[] {
  ...
  return [
    ...
    ...PACKAGE_ROOTS,
    prefix
      ? join(resolve(prefix), "lib", "node_modules", "@openai")
      : undefined,
    join(dirname(process.execPath), "..", "lib", "node_modules", "@openai"),
  ].filter((root): root is string => !!root);
}

async function findCodexBinaryInPackage(
  packageDir: string,
): Promise<string | undefined> {
        ...
        reason instanceof Error
          ...
          : new Error(
              typeof reason === "string"
                ? reason
                : "Codex web search was cancelled.",
            );
      finish(() => reject(error));
    };
    ...
    if (options.timeoutMs !== undefined) {
      ...
      timeoutId = setTimeout(() => {
        ...
        finish(() => {
          reject(
            new Error(
              `Codex web search timed out after ${Math.ceil(timeoutMs / 1000)} seconds.`,
            ),
          );
        });
      }, timeoutMs);
