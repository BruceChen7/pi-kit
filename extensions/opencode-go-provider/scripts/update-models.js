...
// Get display string for input types
function getInputDisplay(inputTypes) {
  if (!inputTypes?.includes("image")) return "Text";
  return inputTypes.includes("text") ? "Text + Image" : "Image";
}
...
function generateReadmeRow(model) {
  const cost = model.cost || {};
  return `| ${model.name} | ${getInputDisplay(model.input)} | ${formatNumber(model.contextWindow)} | ${formatNumber(model.maxTokens)} | ${formatCost(cost.input)} | ${formatCost(cost.output)} |`;
}
...
${tableRows}`;
  ...
  readme = readme.replace(
    /\*\*\d+ Optimized AI Models\*\*/,
    `**${models.length} Optimized AI Models**`,
  );
...
async function main() {
  ...
  try {
    ...
    const apiModels = Object.values(provider.models).filter(
