...
Review the `git diff --no-ext-diff origin/<base>` output for the issues listed below. Be specific — cite `file:line` and suggest fixes. Skip anything that's fine. Only flag real problems.
...
To do this: use `rg` to find all references to the sibling values (e.g., `rg "lfg|mega"` to find tier consumers). Read each match. This step requires reading code OUTSIDE the diff.
...
#### Completeness Gaps
- Shortcut implementations where the complete version would cost <30 minutes of AI time (e.g., partial enum handling, incomplete error paths, missing edge cases that are straightforward to add)
- Options presented with only human-team effort estimates — should show both human and AI time
