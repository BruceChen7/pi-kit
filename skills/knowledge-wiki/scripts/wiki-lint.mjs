...
function findUngroundedConcepts() {
  ...
  const existingSummaries = new Set(
    findSummaryFiles(SUMMARIES_DIR).map((file) => relToKnowledge(file)),
  );
  const ungrounded = [];
  ...
  for (const entry of fs.readdirSync(CONCEPTS_DIR, { withFileTypes: true })) {
    ...
    const content = fs.readFileSync(
      path.join(CONCEPTS_DIR, entry.name),
      "utf8",
    );
    ...
    const validSourceCount = sourceBullets
      .flatMap((line) => {
        const targets = [];
        const re = /\[\[((?:[^\]|]|\](?!\]))+)(?:\|[^\]]+)?\]\]/g;
        for (const match of line.matchAll(re)) targets.push(match[1].trim());
        return targets;
      })
      .filter(
        (target) =>
          target.startsWith("Wiki/Summaries/") &&
          existingSummaries.has(`${target}.md`),
      ).length;
...
function findSelfLinks() {
  ...
  for (const entry of fs.readdirSync(CONCEPTS_DIR, { withFileTypes: true })) {
    ...
    if (
      bullets.some(
        (line) => line.includes(selfLinkMarker) || line.includes(selfLinkExact),
      )
    ) {
  ...
  function scanFile(fullPath, relPath) {
    ...
    for (const line of lines) {
      ...
      if (
        !inKeyC ||
        !line.startsWith("- ") ||
        !stripped.startsWith("[[Wiki/Concepts/")
      )
        continue;
    ...
    const duplicates = Object.entries(conceptLines)
      ...
      .map(([conceptPath, linesForConcept]) => ({
        conceptPath,
        lines: linesForConcept,
      }));
