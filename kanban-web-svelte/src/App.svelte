...
let baseUrl = isBrowser
  ? (window.localStorage.getItem("kanban.runtime.baseUrl") ??
    "http://127.0.0.1:17888")
  : "http://127.0.0.1:17888";
let token = isBrowser
  ? (window.localStorage.getItem("kanban.runtime.token") ?? "")
  : "";
...
$: if (isBrowser) {
  window.localStorage.setItem("kanban.runtime.baseUrl", baseUrl);
  window.localStorage.setItem("kanban.runtime.token", token);
}
...
function createApi(): KanbanRuntimeApi {
  if (!baseUrl.trim()) {
    throw new Error("Runtime base URL is required");
  }
  if (!token.trim()) {
...
<main class="app">
  <section class="panel">
    ...
    <div class="settings-grid">
      ...
      <input id="base-url" bind:value={baseUrl} placeholder="http://127.0.0.1:17888" />

      <label for="token">Token</label>
      <input id="token" bind:value={token} placeholder="Bearer token" />
    </div>
          ...
          <article class="lane">
            ...
            <ul class="cards">
              {#each lane.cards as card}
                <li>
                  <button class="card-btn" on:click={() => void openActionDialog(card)}>
