...
async function showSnippetList(
  snippets: Snippets,
  ...
  if (names.length === 0) {
    ctx.ui.notify(
      "No snippets defined. Use `/snippet add` to create one.",
      "info",
    );
    return;
  }
  ...
  await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
    ...
    container.addChild(
      new Text(theme.fg("accent", theme.bold("Snippet List"))),
    );
    ...
    container.addChild(
      new Text(
        theme.fg("dim", "Ctrl+K/J navigate • enter to insert • esc cancel"),
      ),
    );
    container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
...
async function deleteSnippetPrompt(
  name: string,
  ctx: ExtensionContext,
): Promise<boolean> {
...
async function showManagementSelector(
  snippets: Snippets,
  ctx: ExtensionContext,
): Promise<void> {
  ...
  const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
    ...
    container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
    container.addChild(
      new Text(theme.fg("accent", theme.bold("Manage Snippets"))),
    );
    ...
    container.addChild(
      new Text(
        theme.fg("dim", "Ctrl+K/J navigate • enter select • esc cancel"),
      ),
    );
    container.addChild(new DynamicBorder((str) => theme.fg("accent", str)));
        ...
        const actionItems: SelectItem[] = [
          {
            ...
            description: "Add snippet text at cursor",
          },
          { value: "edit", label: "Edit", description: "Modify this snippet" },
          {
            value: "delete",
            label: "Delete",
            description: "Remove this snippet",
          },
        ];

        const action = await ctx.ui.custom<string | null>(
          (tui2, theme2, _kb2, done2) => {
            const container2 = new Container();
            container2.addChild(
              new DynamicBorder((str) => theme2.fg("accent", str)),
            );
            container2.addChild(
              new Text(theme2.fg("accent", theme2.bold(`Snippet: ${result}`))),
            );
...
export function registerSnippetCommand(pi: ExtensionAPI): void {
  pi.registerCommand("snippet", {
    description:
      "Manage prompt snippets. Use /snippet to browse, add, edit, delete.",
      ...
      switch (parsed.subcommand) {
        ...
        case "add": {
          ...
          if (parsed.text !== undefined && parsed.text !== "") {
            const updatedSnippets = addSnippet(
              snippets,
              parsed.name,
              parsed.text,
            );
