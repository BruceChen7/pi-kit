...
export const NODE_TYPE_CATALOG: NodeTypeEntry[] = [
  ...
  },
  {
    ...
    props: {
      ...
      pin: "string[] (optional) — diff mode only: entry names ranked first, in declared order (unmatched names are ignored; the rest follow the impact ranking). The focus mechanism for the capped entry list.",
    ...
    guidelines: [
      ...
      "pin ranks listed entry names first (declared order) — use it to surface the key production flows when the capped entry list would bury them under tests/mocks. Without pin, entries rank by change impact.",
