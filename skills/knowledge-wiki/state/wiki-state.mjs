 ...
 *
 * Usage:
 *   node skills/knowledge-wiki/state/wiki-state.mjs find-unprocessed-summaries <skill-name>
 *   node skills/knowledge-wiki/state/wiki-state.mjs set-last-run <skill-name>
 *   node skills/knowledge-wiki/state/wiki-state.mjs dismiss-pair <skill-name> <pathA> <pathB>
 *   node skills/knowledge-wiki/state/wiki-state.mjs prune-merge-pairs
 *   node skills/knowledge-wiki/state/wiki-state.mjs prune-cluster-pairs
 *
...
if (!subcommand) {
  console.error(
    "Usage: node skills/knowledge-wiki/state/wiki-state.mjs <find-unprocessed-summaries|set-last-run|dismiss-pair|prune-merge-pairs|prune-cluster-pairs> [args]",
  );
  process.exit(1);
}
...
if (subcommand === "find-unprocessed-summaries") {
  ...
  if (!skillName) {
    console.error(
      "Usage: node skills/knowledge-wiki/state/wiki-state.mjs find-unprocessed-summaries <skill-name>",
    );
    process.exit(1);
  }
  ...
  if (!skillName) {
    console.error(
      "Usage: node skills/knowledge-wiki/state/wiki-state.mjs set-last-run <skill-name>",
    );
    process.exit(1);
  }
  if (!LAST_RUN_SKILLS.has(skillName)) {
  ...
  if (!skillName || !pathA || !pathB) {
    console.error(
      "Usage: node skills/knowledge-wiki/state/wiki-state.mjs dismiss-pair <skill-name> <pathA> <pathB>",
    );
    process.exit(1);
  }
